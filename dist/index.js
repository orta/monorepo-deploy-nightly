...
function getPackageMetadata(changedPackages, settings) {
    ...
    changedPackages.forEach(packagePath => {
        const root = path_1.join(settings.cwd, 'packages', packagePath);
        const packageJSONPath = path_1.join(root, 'package.json');
        const json = JSON.parse(fs_1.readFileSync(packageJSONPath, 'utf8'));
...
function getChangedPackages(files) {
    ...
    files.split(os_1.EOL).forEach(path => {
        if (!path.length)
            return;
...
const runner_1 = __webpack_require__(39);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const only = core.getInput('only') ? JSON.parse(core.getInput('only')) : [];
...
const getPackageVersion = (packageMD) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const npmInfo = yield axios({ url: `https://registry.npmjs.org/${packageMD.name}`, method: "GET" });
